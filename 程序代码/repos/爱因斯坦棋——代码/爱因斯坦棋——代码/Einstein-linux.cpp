...
string GetSystemTime()
{
	...
	sprintf(szDateTime, "%02d-%02d-%02d %02d:%02d:%02d", t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
...
string getSystemTime()
{
	...
	t=localtime(&tt);
	char szDateTime[100] = { 0 };
	sprintf(szDateTime, "%02d-%02d-%02d", t->tm_year+1900, t->tm_mon+1, t->tm_mday);
	string time(szDateTime);
	return time;
...
int Einstein::parse(std::string s)
{
	...
	else dice = s[s.length() - 1] - '0';//判断蓝或红方
	for(int i=0;i<5;++i)
		for(int j=0;j<5;++j)
		{
			position temp;
...
mydefend defend(int i, int op)
{
	...
	mydefend def; def.defend1 = defend1; def.defend2 = defend2;
	return def;
}
...
int caculate(int a, mydefend d, int k, int i)//i为棋号
{
	int x = mychess.find(i)->second.X;
	int y = mychess.find(i)->second.Y;
	if(me==0)//红方
	{
		if(x+y<=4&&mychess.size()<=4)//仍处于我方阵营且棋局已进行
		{
			return 30 * a + 2 * d.defend1 + 3 * d.defend2  + k;//以防守为主
		}
		else if (x + y > 4&&x+y<=6)//处于敌方阵营
			return 30 * a + 3 * d.defend1 + d.defend2 + k;//往敌人较少地方走
		else if (x + y > 6) return 50 * a + d.defend1 + d.defend2 + k;
		else return 30 * a + d.defend1 + d.defend2 + k;
	}
	if(me==1)//蓝方
	{
		if (x + y >= 4 && mychess.size() <= 4)//仍处于我方阵营且棋局已进行
		...
		if (x + y < 4 && x + y >= 2)//处于敌方阵营
			return 30 * a + 2 * d.defend1 + d.defend2 + k;//往敌人较少地方走
		if (x + y < 2) return 50 * a + d.defend1 + d.defend2 + k;
		else return 30 * a + d.defend1 + d.defend2 + k;
	}
}
ope_x judge(int number)//传入色子数
{
	...
	if(mychess.find(number)!=mychess.end())//存在该棋子
	{
		...
		if(debug(number,1))//可以这样走
		{
			...
			a.val = caculate( attack(number, 1) , defend(number, 1) , kill(number, 1),number);//先直接取和
			judges.push_back(a);
		}
		if (debug(number, 2))//可以这样走
		{
			b.val = caculate(attack(number, 2), defend(number, 2), kill(number, 2),number);//先直接取和
			judges.push_back(b);
		}
		if (debug(number, 3))//可以这样走
		{
			c.val = caculate(attack(number, 3), defend(number, 3), kill(number, 3),number);//先直接取和
			judges.push_back(c);
		}
	}
	else//不存在该棋子
	{
		if (mychess.begin()->first > number)
		{
			...
			{
				a.val = caculate(attack(num, 1), defend(num, 1), kill(num, 1),num);//先直接取和
				judges.push_back(a);
			}
			if (debug(num, 2))//可以这样走
			{
				b.val = caculate(attack(num, 2), defend(num, 2), kill(num, 2),num);//先直接取和
				judges.push_back(b);
			}
			if (debug(num, 3))//可以这样走
			{
				c.val = caculate(attack(num, 3), defend(num, 3), kill(num, 3),num);//先直接取和
				judges.push_back(c);
			}
		}
		else
		{
			...
			else//存在两颗能走的棋
			{
				...
				{
					a.val = caculate(attack(num1, 1), defend(num1, 1), kill(num1, 1),num1);//先直接取和
					judges.push_back(a);
				}
				if (debug(num1, 2))//可以这样走
				{
					b.val = caculate(attack(num1, 2), defend(num1, 2), kill(num1, 2),num1);//先直接取和
					judges.push_back(b);
				}
				if (debug(num1, 3))//可以这样走
				{
					c.val = caculate(attack(num1, 3), defend(num1, 3), kill(num1, 3),num1);//先直接取和
					judges.push_back(c);
				}
				if (debug(num2, 1))//可以这样走
				{
					d.val = caculate(attack(num2, 1), defend(num2, 1), kill(num2, 1),num1);//先直接取和
					judges.push_back(d);
				}
				if (debug(num2, 2))//可以这样走
				{
					e.val = caculate(attack(num2, 2), defend(num2, 2), kill(num2, 2),num2);//先直接取和
					judges.push_back(e);
				}
				if (debug(num2, 3))//可以这样走
				{
					f.val = caculate(attack(num2, 3), defend(num2, 3), kill(num2, 3),num2);;//先直接取和
					judges.push_back(f);
				}
			}
		}
